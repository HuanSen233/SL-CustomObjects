...
public static class CurvePlacementHelper
{
    ...
    /// <param name="i">小线段索引（0 起）</param>
    /// <param name="pts">2D 采样点（SamplePoints() 的结果，须与 pts3d 同步）</param>
    /// <param name="pts3d">3D 采样点（SamplePoints3D() 的结果；非 3D 曲线传 null）</param>
    /// <param name="pos">输出：世界位置（段中心 + 偏移）</param>
    /// <param name="rot">输出：世界旋转（LookRotation + 旋转偏移）</param>
    /// <param name="scale">输出：世界缩放（BaseScale → FitSegmentLength → RelativeScale）</param>
